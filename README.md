...
- 💬 **聊天式交互**：像发微信一样下达指令，AI 自动判断「生成新文档 / 按模板填写 / 编辑已有文档」；聊天区展示大纲与修改计划，确认后再落笔；结果自带「打开文档 / 所在文件夹」按钮，支持连续对话与时间戳。
...
- ✍️ **过短章节自动补全**：生成模式下若发现「没写完/只有一句话」的章节，自动做一次批量扩写（说明类补足要点、步骤类补全编号流程），可用 `WORDAGENT_AUTO_EXPAND=0` 关闭以省 token。
...
# 4. 直接在聊天框输入要求，例如：
#    写一份《数据库课程实验报告》          -> 自动生成新文档
#    点「📎 附件」添加 .docx 后说：把标题改成《XXX》  -> 自动识别为编辑
#    点「📎 附件」添加模板后说：按模板填写这份报告    -> 自动识别为模板填写
```
